...
func TestPrompt_EmptyContent(t *testing.T) {
	...
	// Stub HTTP client to return content with empty text
	body := `{"content":[{"text":""}]}`
	pEmpty := NewWithClient(&fakeClient{resp: &http.Response{
		StatusCode: http.StatusOK,
		Body:       io.NopCloser(bytes.NewBufferString(body)),
	}})
	_, err := pEmpty.Prompt("claude-2.1", "prompt")
	if err == nil || !strings.Contains(err.Error(), "no content in response from anthropic") {
...
func TestPrompt_HTTPStatusError(t *testing.T) {
	...
	_, err := pStatusErr.Prompt("claude-2.1", "prompt")
